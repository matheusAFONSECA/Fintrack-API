...
def test_alert_visualization_missing_email():
    ...
    assert response.status_code == 400, f"Expected 400 for missing email parameter, got {response.status_code}"
...
def test_alert_visualization_success():
    ...
    assert register_response.status_code == 200, "Registro falhou durante o teste de visualização de alertas."
    ...
    response = visualize_alert(params)
    assert response.status_code == 200, f"Expected 200 for successful alert visualization, got {response.status_code}"
    json_response = response.json()
